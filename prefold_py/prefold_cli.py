...
def coloured_seq (seq_dict, tag, disorder_dict):
    ...
    print (new_string.replace('red', '\033[31m').replace('green', '\033[32m').replace('reset', '\033[0m'))
...
def main():
    ...
    optional.add_argument('-b', '--boundry',
        help='Specify the boundry for calling disordered regions of peptide sequence; Default=0.005.',
        type=float, default=0.005)
    optional.add_argument('-ter', '--ter_include',
        help='Specify the flag for N and C terminal charges to be NOT included in the calculation.',
        action='store_false')
    ...
    optional.add_argument('-pka', '--pka_table',
        help='Specify a file with amino acid residue pKa values to be used for the \
        calculation (must be located in datasets folder).', default='PKA_DATA_VOET.DAT')
    ...
    for key, value in sequence_dict.copy().items():
        if len(value) > args.window_size: # checks if the sequence len is larger than win size
            data_dict[key] = unfoldty_sliding_win (value, args.ph_lvl,
                ...
                # returns pd dataframe with hb, charge and unfold column; stores in a dict under the fasta tag as a key
        else:
